...
> Статус: FROZEN для прототипирования (2026-09-15). Не «завершённый дизайн».
> Заморозка значит: грамматики не меняем до визуальной проверки A1–A3.
> Порядок: Grammar → A1–A3 → визуальная проверка → исправление grammar
> при необходимости → только потом store/attention/world mapping.
...
A11 — Понятность без документации: состояние Юны (говорит / занята /
  наблюдает / спит) считывается без чтения грамматик. Внутренняя механика
  (AssistantStatus, Attention.focus) при этом знать не требуется.
...
Проверки на прототипе (что смотреть глазами, не кодом):

T1 — CENTER→RIGHT как перемещение, не переключение экранов.
  Главный тест Living Stage. Сцена не пересобирается: слот едет,
  реплика остаётся, pocket раскрывается. Если ощущается как новая
  страница — провал, правим сцену, не анимации.
T2 — REACTION не конкурирует с INFORMATION. Зелёный glow = отношение Юны
  (рада, готово), а не статус процесса. Статус процесса — только в pocket
  текстом и галочками. Пользователь не должен гадать, что означает свет.
T3 — WORKING живой, но Юна не индикатор загрузки. Pocket показывает задачу,
  Юна — присутствие и отношение (LEAN к pocket, спокойствие, не пульсация
  в такт процентам). Тонкая граница — её выявит только прототип.
T4 — OBSERVING почти незаметен. Никакой театральности:
  чуть ушла влево, чуть темнее, молчит. Среда меняет поведение, не объявляет.
T5 — SLEEPING как состояние сущности, не «заставка включена».
  Решают DIM, уменьшенный слот и одна тихая подпись. Будится одной репликой.
