...
3. Graphiti end-to-end recall — после Pass 1 → **кандидат Phase 2 → Step 1 подтверждён гэпом**: остаётся открытым (протокол VERIFIED: initialize + 3 tools; реальный recall не подтверждён. **2026-09-16 реальный E2E-прогон через mcp-adapter (qwen3:1.7b + nomic-embed): memory_add падает — `ExtractedEdges: edges Field required` (structured output локальной модели); сервер предупреждает: Kuzu-бэкенд deprecated (upstream не поддерживается). См. docs/research/phase-2-memory-research.md §5.** Блокер остаётся: Python sidecar + LLM-провайдер.
