...
## DEC-012 — Один коммит = одна задача; файлы дизайнера — отдельные коммиты

- **Decision:** коммит содержит изменения ровно одной задачи. Файлы UI-дизайнера
  (`docs/design/**`, работа Muse Spark 1.3 в общем дереве) коммитятся только отдельным
  дизайн-коммитом и **никогда не подхватываются автоматически** в кодовые коммиты
  (в т.ч. через `git add -A` — перед ним обязательна проверка `git status --short`).
- **Date:** 2026-09-16.
- **Problem:** в `3206003` (P1-1 migration tests) случайно оказался bundle `docs/design/` —
  замусорил монотопичность; выявлено ревью GPT.
- **Chosen:** перед каждым коммитом — явный просмотр `git status`; чужие/untracked файлы
  (параллельные агенты) не индексируются; дизайн-файлы — только по явной задаче дизайн-трека.
  verify-manifest может показывать их как «лишние» — это ожидаемо, не ошибка.
- **Evidence:** ревью `3206003` (GPT); урок в `docs/JOURNAL.md` (`f3c8f67`).
- **Status:** Accepted.
- **Superseded by:** —.

## DEC-013 — Роли разработки: lead + исследователь + кодер + контекст-архивариус

- **Decision:** разделение ролей в разработке U.N.A.:
  (1) **Lead (человек) + исследовательский AI** — исследование, идеи, архитектурные
  решения, постановка задач;
  (2) **AI-кодер** — реализация, тесты, git, verification;
  (3) **контекст-архивариус (второй AI)** — превращение диалогов/решений/исследований
  в структурированный канонический контекст.
- **Date:** 2026-09-17.
- **Problem:** один агент не может одновременно быть архитектором, программистом,
  архивариусом и держателем «почему три недели назад решили не делать»; чат-контекст
  теряется между сессиями.
- **Chosen:** контекст-архивариус работает строго по конвейеру
  `диалог → извлечение → классификация → предложение изменения → lead утверждает →
  canonical docs` и НЕ является источником истины (см. DEC-010: иерархия неизменна).
  Формат предложения: `DISCOVERED / CONFLICT / PROPOSED UPDATE / STATUS: PENDING APPROVAL`;
  изменения попадают в canonical docs (decision-log, implementation-map, research-backlog,
  AGENTS.md) только после утверждения lead'ом. Целевая петля: исследование → структурирование →
  реализация кодером → verification → возврат фактов в документы → следующая сессия
  стартует с нового состояния (живая engineering memory: контекст проекта как
  структурированное состояние, а не transcript).
- **Evidence:** роли подтверждены lead'ом 2026-09-17; артефакты конвейера уже созданы
  (`docs/Phase2/UNA_Phase2_Step1.5_Routing_Reconciliation_Brief.md`,
  `docs/research/system-one-decision-models.md`, Pass 6 в research-backlog);
  DEC-011 (GitHub как оперативная память) — смежная основа.
- **Status:** Accepted.
- **Superseded by:** —.
